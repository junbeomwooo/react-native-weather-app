...
export function getWeatherIconsforMaps(
  weatherId: number,
  size: number,
  theme: "light" | "dark" | null
) {
  ...
  if (storm?.includes(weatherId)) {
    return <Ionicons name="thunderstorm-outline" size={size} color="black" className="opacity-50" />;
  } else if (drizzle?.includes(weatherId) || rain?.includes(weatherId)) {
    return <Ionicons name="rainy-outline" size={size} color="black" className="opacity-50" />;
  } else if (snow?.includes(weatherId)) {
    return <Ionicons name="snow-outline" size={size} color="black" className="opacity-50" />;
  } else if (mist?.includes(weatherId)) {
    return (
      <MaterialCommunityIcons name="weather-fog" size={size} color="black" className="opacity-50" />
    );
  } else if (clear?.includes(weatherId)) {
    const clear =
      theme === "light" ? (
        <Fontisto name="day-sunny" size={size} color="black" className="opacity-50" />
      ) : (
        <Ionicons name="moon-outline" size={size} color="black" className="opacity-50" />
      );
    return clear;
  } else if (fewClouds?.includes(weatherId)) {
    const fewClouds =
      theme === "light" ? (
        ...
        <Ionicons name="cloudy-night-outline" size={size} color="black"
        className="opacity-50" />
      );
